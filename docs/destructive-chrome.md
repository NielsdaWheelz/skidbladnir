...
| F3 | High | ` · RE-PAIR` is the only imperative in a vocabulary of state tokens, and it names a remedy that no longer exists — the pairing screen was deleted in the multi-machine cut. Every sibling surface says "provisioning repair". | Logged → ` · IDENTITY CHANGED` |
