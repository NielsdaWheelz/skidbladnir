...
resolved when: the installed skid-owned command and private home are verified on macbook, devbox, and arch; approved live claude sessions on each host show correct native inspection, bounded history and matched stop behavior; helper failures retain honest terminal fallback; no herdr home, command, or state is accessed. record content-free results and remove this file.
